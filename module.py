...
def label_smoothed_nll_loss(lprobs, target, epsilon, ignore_index=None, sum=True):
    ...
    nll_loss = -lprobs.gather(dim=-1, index=target.to(torch.int64))
...
class MonoEncoder(nn.Module):
    ...
    def forward(self, input_ids):  # input_ids -> seq_len x (num_mem_sents * bsz)
        src_repr = self.embed_scale * self.src_embed(input_ids) + self.src_pos_embed(input_ids)  # src_repr -> seq_len x (num_mem_sents * bsz) x dim
        src_repr = F.dropout(src_repr, p=self.dropout, training=self.training)
        src_mask = torch.eq(input_ids, self.vocab.padding_idx)  # src_mask -> seq_len x (num_mem_sents * bsz)
