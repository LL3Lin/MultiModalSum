...
class Transformer(nn.Module):
    ...
    def forward(self, x, kv=None,
                ...
                external_memories=None, external_padding_mask=None, attn_bias=None):
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, kv=None,
                ...
                need_weights=None, attn_bias=None):
...
class DecoderTransformer(nn.Module):
    ...
    def __init__(self, layers, embed_dim, ff_embed_dim, num_heads, dropout, with_external=False, weights_dropout=True):
        ...
        for _ in range(layers):
            self.layers.append(
                DecoderTransformerLayer(embed_dim, ff_embed_dim, num_heads, dropout, with_external, weights_dropout))

    def forward(self, x, kv=None,
                self_padding_mask=None, self_attn_mask=None,
                external_ast_memories=None, external_ast_padding_mask=None,
                external_mem_memories=None, external_mem_padding_mask=None):
        for idx, layer in enumerate(self.layers):
            x, _, _ = layer(x, kv, self_padding_mask, self_attn_mask, external_ast_memories, external_ast_padding_mask,
                            external_mem_memories, external_mem_padding_mask)
        return x
...
class DecoderTransformerLayer(nn.Module):
    ...
    def __init__(self, embed_dim, ff_embed_dim, num_heads, dropout, with_external=False, weights_dropout=True):
        ...
        self.dropout = dropout
        if self.with_external:
            self.external_ast_attn = MultiheadAttention(embed_dim, num_heads, dropout, weights_dropout)
            self.external_ast_layer_norm = nn.LayerNorm(embed_dim)
            self.external_mem_attn = MultiheadAttention(embed_dim, num_heads, dropout, weights_dropout)
            self.external_mem_layer_norm = nn.LayerNorm(embed_dim)
    ...
    def forward(self, x, kv=None,
        ...
        if self.with_external:
            residual = x
            x, _ = self.external_ast_attn(query=x, key=external_ast_memories, value=external_ast_memories,
                                          key_padding_mask=external_ast_padding_mask,
                                          need_weights=need_weights)
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = self.external_ast_layer_norm(residual + x)

            residual = x
            x, external_attn = self.external_mem_attn(query=x, key=external_mem_memories, value=external_mem_memories,
                                                      key_padding_mask=external_mem_padding_mask,
                                                      need_weights=need_weights)
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = self.external_mem_layer_norm(residual + x)
